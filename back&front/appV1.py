...
def delete_history_by_id(history_id):
    try:
        # Convert string ID to int if your IDs are integers
        # history_id = int(history_id)  # Uncomment if using numeric IDs
        ...
        if item_to_delete:
            analysis_history.remove(item_to_delete)
            return jsonify({"success": True})
        else:
            return jsonify({"success": False, "error": "ไม่พบข้อมูลที่ต้องการลบ"}), 404
            
    except Exception as e:
        print(f"Error during deletion by ID: {e}")
        return jsonify({"success": False, "error": "เกิดข้อผิดพลาดขณะลบข้อมูล"}), 500
...
def get_history_data():
    ...
    sorted_history = sorted(analysis_history, key=lambda x: x.get("timestamp", "1970-01-01 00:00:00"), reverse=True)
...
def save_history():
    try:
        ...
        if not data.get("text"):
            return jsonify({"success": False, "error": "ข้อความไม่ถูกต้อง"}), 400
            
        # Generate a unique ID for the history item (you can use a more sophisticated method)
        import uuid
        ...
        return jsonify({"success": False, "error": "เกิดข้อผิดพลาดขณะบันทึกข้อมูล"}), 500
